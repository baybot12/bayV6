...
* Author MhankBarBar, www.youtube.com/abdulaziz albaihaqi*
...
switch(command) {
				...
				case 'menu':
					...
					await costum(help(pushname, prefix, botName, ownerName, reqXp, getLevelingLevel, sender, _registered, uangku), text, tescuk, cr,fs.readFileSync)
                ...
                if (isBanned) return reply(ind.baned())					
					...
					case 'afk':
                                        ...
                                        baby.sendMessage(from, fku, text, {quoted: mek})

				const afk = fs.readFileSync('./assets/afk');

                baby.sendMessage(from, afk, MessageType.image, {quoted: mek, caption: '*KAKA INI  SEDANG AFK  JANGAN DI GANGGU YA!!*'})
				...
				case 'unafk':
                                        ...
                                        baby.sendMessage(from, kl7, text, {quoted: mek})
...
*/
		...
		case 'katakatadilan':
			...
			if (!isRegistered) return reply(ind.noregis())
				...
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo  MrG{108P}& Gans`)
		...
		anu = await fetchJson(`https://api.vhtear.com/ytdl?link=${args[0]}&apikey=${VhtearKey}`, {method: 'get'})
			...
			case 'hentai':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
						...
						baby.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
                	...
                	case 'randomhentai':
					 ...
					 // Fix Bug ByMrG{108P}*X ItsmeikyXSec404				
...
*/
		...
		if (!isGroup) return reply(ind.groupo())
					...
					client.sendMessage(from, '*тЮ╕ Jawaban :* '+anu.result.jawaban+ '\n\nтАв Penjelasan: *'+ anu.result.desk+'*', text, {quoted: mek}) // ur cods
...
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink Rem`)
...
if (isBanned) return reply(ind.baned())
				...
				reply(ind.wait())
					...
					baby.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
...
case 'anime':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					baby.sendMessage(from, pok, image, { quoted: mek , caption: 'Subscribe:https://youtube.com/channel/UCal0HWOurq6GIF4_z0N2B6Q'})
			...
			if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					baby.sendMessage(from, nye, image, { caption: 'chika waifu partner', quoted: mek })
			...
			if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					baby.sendMessage(from, nye, image, { caption: 'Puja Paha Rikka >//<', quoted: mek })
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anu = await fetchJson(`https://api.fdci.se/rep.php?gambar=ram`, {method: 'get'})
					...
					baby.sendMessage(from, nye, image, { caption: 'ram', quoted: mek })
			...
			case 'wallpaperty':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/waifu?apikey=BotWeA`, {method: 'get'})
                                        ...
                                        baby.sendMessage(from, buffer, image, {quoted: mek, caption: waifu})
...
tod = await getBuffer(`https://i.ibb.co/DQbWvnV/026d4893040839ffe3d43e76bcffa970.jpg`)
  ...
  baby.sendMessage(from,tod,image, 'Ini waifu owner saya,luh ambil yok gass gelud!!!...',MessageType.text, { quoted: mek} )
...
break
		...
		case 'grenneon':
				...
				aruga = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/greenneon?apikey=WEMPYGANSS&text=${aruga}`)
...
case 'tts':
				...
				case 'hartatata2':
				 ...
				 // Update ByMrG{108P}* & RzkyO & ItsmeikyXSec404				
...
case 'slow':
          ...
          reply('Wait..')
                                       ...
                                       if (args[0].startsWith('81534162316')) return reply('Gagal tidak dapat menelpon nomer owner')
